...
class HrTermination(models.Model):
    ...
    def action_approved(self):
        ...
        self.sudo().employee_id.contract_id.date_end = self.termination_date
        self.sudo().employee_id.contract_id.end_incentive = self.end_incentive

        for contract in self.employee_id.contract_ids:
            if contract.state != 'cancel':
                contract.sudo().date_end = self.termination_date
                contract.sudo().state = 'cancel'
                contract.sudo().end_incentive = self.end_incentive
                contract.sudo().end_incentive_month = self.end_incentive_month
