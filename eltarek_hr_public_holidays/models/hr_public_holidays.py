...
class HrPublicHolidays(models.Model):
    ...
    @api.model
    def throw_lines(self):
        for rec in self:
            if rec.line_ids:
                working_times = self.env['resource.calendar'].search([])
                for work_line in working_times:
                    work_line.global_leave_ids = [(5, 0)]
                    for line in rec.line_ids:
                        work_line.global_leave_ids = [(0, 0, {
                            'name': line.name,
                            'date_from': str(line.date - timedelta(days=1)) + ' 22:00:00',
                            'date_to': str(line.date) + ' 21:59:59',
